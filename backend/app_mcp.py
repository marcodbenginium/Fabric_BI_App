...
from fabric_agent_mcp import ask_fabric_agent_mcp
...
def chat():
    ...
    # MCP è stateless: nessuna gestione di thread/assistant
    result = ask_fabric_agent_mcp(token, prompt)
...
def chat_reset():
    """Azzera history della sessione corrente."""
...
def chat_summary():
    """Genera un riassunto della conversazione usando Phi-3.5 Mini."""
    history = session.get('chat_history', [])
    if not history:
...
def export_pdf():
    ...
    pdf.cell(0, 10, 'Report Conversazione - Fabric Data Agent (MCP)', ln=True, align='C')
    ...
    filename = f'conversazione_mcp_{datetime.utcnow().strftime("%Y%m%d_%H%M")}.pdf'
